...
@Autonomous (name = "BlueRoadRunner", group = "AutoRR")
...
//        }
        ...
        TrajectorySequence trajSeqLow = drive.trajectorySequenceBuilder(trajToCarrousel.end())
                ...
                .splineToConstantHeading(new Vector2d(-31, 24), Math.toRadians(0))

                .UNSTABLE_addTemporalMarkerOffset(-3, () -> {
                    motorArm.setTargetPosition(650);
                    linearServo.setPosition(mapRange(3300, -100, .12, .50, 650));
                    motorArm.setPower(.8);
                })
        ...
        TrajectorySequence trajSeqHigh = drive.trajectorySequenceBuilder(trajToCarrousel.end())
                ...
                //Move to wobble tower
                .strafeTo(new Vector2d(-60, 35))
                .splineToConstantHeading(new Vector2d(-27, 24), Math.toRadians(0))

                .UNSTABLE_addTemporalMarkerOffset(-3, () -> {
                    motorArm.setTargetPosition(2500);
                    linearServo.setPosition(mapRange(3300, -100, .12, .50, 2700));
                    motorArm.setPower(.8);
                })
...
//                .lineTo(new Vector2d(-6, 50))
        ...
        TrajectorySequence trajSeqMidL = drive.trajectorySequenceBuilder(startPoseLeft)
                ...
                //Move to wobble tower
                .waitSeconds(2)
                .splineToConstantHeading(new Vector2d(-12, 38.5), Math.toRadians(-90))
        ...
        TrajectorySequence trajSeqHighL = drive.trajectorySequenceBuilder(startPoseLeft)
                ...
                .UNSTABLE_addTemporalMarkerOffset(-3, () -> {
                    motorArm.setTargetPosition(2700);
                    linearServo.setPosition(mapRange(3300, -100, .12, .50, 2700));
                    motorArm.setPower(.8);
                })
                ...
                if (myPipeline.getRectMidpointX() > 400) {
                    ...
                    if (!isStopRequested()) {
                        ...
                        if(!StartLocation) {
                            ...
                            if (ParkLocation && StartLocation) {
                                telemetry.addData("WM:", "Warehouse");
                                drive.followTrajectorySequence(trajToWarehouse);
                            }
                            if (ParkLocation && !StartLocation) {
                                telemetry.addData("WM:", "Carousel");
                                sleep(5000);
